...
/**
 * Menghapus produk dengan id tertentu dari database
 * @route DELETE /api/product/:id
 * @param {Object} req.params.id Id produk yang ingin dihapus
 * @param {Object} res.json Status code, pesan, dan data produk yang dihapus
 * @returns {void} JSON yang berisi konfirmasi produk telah dihapus
 */
exports.deleteProductById = async (req, res) => {
  try {
    ...
    res.status(200).json({
      statusCode: 200,
      message: `Product with id: ${_id} deleted successfully`,
      data: deletedProduct,
    });
  } catch (error) {
    // Handle error
    res.status(500).json({
      statusCode: 500,
      message: `Failed to delete product with id: ${_id}`,
      error: error.message,
    });
  }
};
